...
filt_path <- "tmp/Lab/TestRun/filtered_TestRun_1/"
#Pipeline filtration 
...
ptable <- read.csv(file = "data/Lab/primer.file.multi.csv", sep=",", header=TRUE, stringsAsFactors=FALSE)
...
if(doMultiAmp){
  ...
  filedir <- "tmp/Lab/TestRun/stratified_filea_test_run_1/"
  ...
  saveRDS(MA, "tmp/Lab/MA_multi_testrun_1.RDS")
} else{
    MA1 <- readRDS("tmp/Lab/MA_multi_testrun_1.RDS")
}
...
#trackingF <- getPipelineSummary(MA1) 
...
#Creation of a folder for filtrated reads 
filt_path <- "tmp/Lab/FullRun/filtered_FullRun_1/"
...
if(doMultiAmp){
  MA <- MultiAmplicon(primer, files)
  filedir <- "tmp/Lab/FullRun/Stratified_files_FullRun_1/"
  if(dir.exists(filedir)) unlink(filedir, recursive=TRUE)
  MA <- sortAmplicons(MA, n=1e+05, filedir=filedir) ## This step sort the reads into amplicons based on the number of primer pairs
  errF <-  learnErrors(unlist(getStratifiedFilesF(MA)), nbase=1e8,
                       verbose=0, multithread = 90)
  ...
  MA <- makeSequenceTableMulti(MA, mc.cores=90) 
  MA <- removeChimeraMulti(MA, mc.cores=90)
  saveRDS(MA, "tmp/Lab/MA_multi_fullrun_1.RDS")
} else{
  MA2 <- readRDS("tmp/Lab/MA_multi_fullrun_1.RDS")
}
...
###New taxonomic assignment for each amplicon.
...
saveRDS(MA1, "tmp/Lab/MATax_TestRun_1_NewTax.Rds")
saveRDS(MA2, "tmp/Lab/MATax_FullRun_1_NewTax.Rds")  
} else {
    MA1 <- readRDS("tmp/Lab/MATax_TestRun_1_NewTax.Rds")
    MA2 <- readRDS("tmp/Lab/MATax_FullRun_1_NewTax.Rds")  
}
...
saveRDS(PS, file="tmp/Lab/PhyloSeqData_All_Tax_New.Rds") ###Results from full + test run 
...
saveRDS(PS.l, file="tmp/Lab/PhyloSeqList_All_Tax_New.Rds") ###For primer analysis
