...
filt_path <- "tmp/Wild/filtered2_1"
...
if(doMultiAmp){
    ...
    filedir <- "tmp/Wild/stratified_files_2_1"
...
#    MA <- derepMulti(MA, mc.cores=90) deprecated, no longer needed
    ...
    saveRDS(MA, "tmp/Wild/MA2_1.RDS")
} else{
    MA <- readRDS("tmp/Wild/MA2_1.RDS")
}
...
saveRDS(MA, file="tmp/Wild/MA2_1Tax.Rds") #

##Start from here after the taxonomic annotation
#MA<- readRDS(file= "tmp/Wild/MA2_1Tax.Rds") ###Test run
##To phyloseq
...
#
saveRDS(PS, file="tmp/Wild/PhyloSeqCombi_HMHZ_2_1.Rds") ###Results from preliminary analysis (Sample data)
#
...
saveRDS(PS.l, file="tmp/Wild/PhyloSeqList_HMHZ_2_1.Rds") ###Full run Pool 3
