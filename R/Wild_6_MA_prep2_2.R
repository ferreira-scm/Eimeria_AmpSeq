...
filt_path <- "tmp/Wild/filtered2_2"
...
if(doMultiAmp){
    ...
    filedir <- "tmp/Wild/stratified_files_2_2"
...
#    MA <- derepMulti(MA, mc.cores=90) deprecated, no longer needed
    ...
    saveRDS(MA, "tmp/Wild/MA2_2.RDS")
} else{
    MA <- readRDS("tmp/Wild/MA2_2.RDS")
}
...
saveRDS(MA, file="tmp/Wild/MA2_2Tax.Rds") #
...
#
saveRDS(PS, file="tmp/Wild/PhyloSeqCombi_HMHZ_2_2.Rds") ###Results from preliminary analysis (Sample data)
#
...
saveRDS(PS.l, file="tmp/Wild/PhyloSeqList_HMHZ_2_2.Rds") ###Full run Pool 3
