...
## let's get all the sequences used in Jarquín-Díaz et al. 2020
...
refEim <- readDNAStringSet("tmp/Eimeria_18S_jarquin2020.fa")
...
names(seqs2) <- c("Lab_standard-fluidic-PCR_ASV1", "Lab_standard-fluidic-PCR_ASV2", "Lab_standard-PCR_ASV3", "Lab_standard-PCR_ASV4")

allSeqs <- c(eim.DB, Eim.f, refEim)
writeFasta(seqs2, "tmp/Lab_ASVs.fa")
writeFasta(allSeqs, "tmp/Eimeria_reference.fa")

Eimeria_ref <-OrientNucleotides(allSeqs)
set.seed(111)
Eimeria_ref <- AlignSeqs(Eimeria_ref, anchor=NA, iterations=20, refinements=20, processors=90)
Eimeria_ref <- AdjustAlignment(Eimeria_ref)
writeFasta(Eimeria_ref, "tmp/reference_tree/Eimeria_reference.fasta")
