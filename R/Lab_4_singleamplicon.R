...
filt_path <- "tmp/Lab/Run18S/filtered18S"
...
ptable <- read.csv(file = "data/Lab/primer.file.csv", sep=",", header=TRUE, stringsAsFactors=FALSE)
...
if(doMultiAmp){
  ...
  filedir <- "tmp/Lab/Run18S/Stratified_files18S"
  ...
  saveRDS(MA, "tmp/Lab/Run18S/MA_single_18S.RDS")
} else{
    MA <- readRDS("tmp/Lab/Run18S/MA_single_18S.RDS")
}
...
saveRDS(PS, file="tmp/Lab/PhyloSeqData18S_SILVA.Rds")
...
saveRDS(PS.l, file="tmp/Lab/PhyloSeqList18S_SILVA.Rds")
