...
## All runs pooled
all.PS.slv <- readRDS("tmp/Lab/PhyloSeqData_All_Tax_New.Rds")
...
allTSS.slv <-  transform_sample_counts(f.all.lp.slv, function(x) x / sum(x)) # multi amplicon
TSS.wang <- transform_sample_counts(f.all.l.slv[[37]], function(x) x / sum(x)) #"wang" amplicon from multi amplicon

## now rescale to sample DNA concentration
...
Eim2 <- subset_taxa(T.sin18.slv, Genus%in%"g__Eimeria") # for single amplicon
Eim_T.all <- subset_taxa(T.all.slv, Genus%in%"g__Eimeria") # for multi amplicon pooled
Eim.TSSw <- subset_taxa(TSS.wang, Genus%in%"g__Eimeria") # for multi amplicon but with TSS transformed
Eim <- subset_taxa(T.wang, Genus%in%"g__Eimeria") # for multi amplicon but only one amplicon
